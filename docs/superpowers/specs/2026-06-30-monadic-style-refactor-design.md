...
- **Topic:** Finish the Effect adoption the codebase already started — replace _escaping_
...
3. **Drive/Dropbox throws — leave entirely as-is.** Their ~15 inner throws are caught by
   ...
   `UploadOutcome.reason` strings (`drive HTTP 500: …`). That _is_ the functional pattern
...
`Option` is a tool for _composable absence in domain logic_, not a blanket replacement for
`null`. Decision rule:
...
subclass; the `message` _field_ populates `Error.prototype.message`, so the existing
...
3. _Optional, only if envelope-compatible:_ throw Convex-native
...
Where a converted function previously had a caller that _threw_ on the `null`, lift that
...
| 5 _(optional)_          | background/entrypoints `Option` (`settleProbe` gone-vs-threw, `sendTabMessage`, `recoverSyndicationBody`, `lastUploadError`) | `background/*`, `entrypoints/*`                            | existing                              |
| 6 _(optional/deferred)_ | Effect spans/annotations in already-Effect code                                                                              | strategies, queue, settings, convex port, `handleDownload` | n/a                                   |
...
// after
const mediaKeyFromUrl = (url: string): Option.Option<string> => Option.fromNullable(parseKey(url))
