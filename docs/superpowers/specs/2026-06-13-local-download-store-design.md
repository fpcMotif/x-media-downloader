...
X Media Downloader is a **local-only** extension, yet it keeps **no durable local record of what you've downloaded**. The popup's download Monitor is a *Snapshot* derived from `session:metrics`, which is ephemeral by design — it resets on service-worker recycle and on browser restart (ADR-0005). The only durable history that exists is **remote**: the opt-in Convex Cloud Sync mirrors download-state metadata into the user's deployment (`media_state`).
...
```
media_state: { requestId, deviceId, lastKind: queued|completed|failed, at, media?: {tweetId,handle,type,url,ext,index} }
```
...
- **Download Record** — one durable, local record of a Save Request's outcome: `requestId`, the original media `url` (link), `filename`, `status` (`queued`/`completed`/`failed`), provenance (`tweetId`, `handle`, `type`, `ext`, `index`), `bytesReceived?`, `bytesTotal?`, `queuedAt`, `finishedAt?`. The local-first twin of a Convex `media_state` row.
...
- **`store.ts`** — pure reducer over `DownloadStore` (`{ records: DownloadRecord[] }` or a keyed map): `upsert(store, record)` (dedupe by `requestId`, newest-first), `capHistory(store, max=500)` (ring-evict oldest), `applyTransition(store, requestId, kind, at, bytes?)` with **status monotonicity** (a terminal `completed`/`failed` never regresses to `queued`), and `decodeStore(raw)` (corrupt → empty, same posture as `decodeOutbox`).
...
- `local:downloadHistory` — the durable `DownloadStore`, bounded (default 500), deduped by `requestId`, newest-first. Sidecar `.json` requests (id `<media-id>.json`, no `MediaItem`) are **excluded**, identical to how the Convex mirror excludes them.
...
- Corrupt stored value ⇒ `decodeStore` recovers to empty.
...
2. `store.ts` — `upsert` dedupes by requestId; cap ring-evicts oldest; `applyTransition` monotonicity (completed↛queued); `decodeStore` corrupt→empty; injected time.
