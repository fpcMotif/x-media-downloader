...
1. **Clearing the browser** — a *new* "Clear entire list" action (auto-scroll the whole Likes/Bookmarks list, un-like/un-bookmark every post, no download), plus the existing per-surface clear-on-save toggles, plus buttons to wipe the extension's own stored data.
...
### Problem
[`handleClearVisible`](../../../src/entrypoints/overlay.content/handlers.ts) clears only the ~30 posts currently mounted, because X virtualizes the timeline. To clear an *entire* list you must auto-scroll it, clearing each post as it mounts, until the bottom.
...
- **Single `confirm()` gate** spelling out scope: *"Un-like / un-bookmark EVERY post on this {Likes|Bookmarks} list by scrolling through all of it? This can affect hundreds of posts and cannot be undone."*
