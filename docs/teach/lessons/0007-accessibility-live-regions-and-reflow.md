...
### C. Persistent Output Node in ConfirmStrip
- **Cause:** `ConfirmStrip` returned a Fragment (`<>`) when idle and a `<div>` when active.
- **Problem:** React reconciliation destroyed the old DOM node and created a new one. The live region was lost during the switch.
- **Fix:** Move the `<output key="confirm-strip-output">` element to the top level outside the condition.
- **Result:** React keeps the same `<output>` element in the DOM when the component activates.
...
- **Keep live regions persistent:** Mount `<output aria-live="polite" aria-atomic="true">` on initial render. Change only its text.
- **Use valid HTML nesting:** Do not place block elements (`<p>`, `<div>`) inside `<output>` tags.
- **Preserve React DOM nodes:** Keep live regions outside conditional branches so React reconciliation retains the DOM node.
