...
function ConvexSyncGroup({ settings, update }: Omit<PanelProps, 'reload'>) {
...
function CloudUploadGroup({ settings, update, reload }: PanelProps) {
    ...
    >
      ...
      {settings.cloudUploadEnabled && (
        <>
          ...
          {connectMsg && <p className="text-sm leading-snug text-muted-foreground">{connectMsg}</p>}
          {cloudStatus && (
...
export function CloudPanel({ settings, update, reload }: PanelProps) {
  return (
    <>
      ...
      />
      <ConvexSyncGroup settings={settings} update={update} />
      <CloudUploadGroup settings={settings} update={update} reload={reload} />
    </>
  )
}
