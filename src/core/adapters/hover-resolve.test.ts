...
// The `grab-target-stale` detail vocabulary (#92): WHEN the fire-time hold
...
describe('staleWhy', () => {
  ...
  it("is 'detached' once the node left the document", () => {
    const wrapper = mount(el('<div><img></div>'))
    const img = wrapper.querySelector('img')!
    img.style.pointerEvents = 'none'
    img.getBoundingClientRect = rect(0, 0, 100, 100)
    img.remove()
    expect(staleWhy(img, [wrapper], 50, 50)).toBe('detached')
  })

  it("is 'not-in-stack' for an interactive img the stack does not reach", () => {
    const wrapper = mount(el('<div><img></div>'))
    expect(staleWhy(wrapper.querySelector('img')!, [mount(el('<div></div>'))], 50, 50)).toBe(
      'not-in-stack',
    )
  })

  it("is 'rect-miss' when a cloaked img's rect no longer covers the point", () => {
    const wrapper = mount(el('<div><img></div>'))
    const img = wrapper.querySelector('img')!
    img.style.pointerEvents = 'none'
    img.getBoundingClientRect = rect(0, 0, 100, 100)
    expect(staleWhy(img, [wrapper], 150, 50)).toBe('rect-miss')
  })

  it("is 'not-in-stack' when nothing in the stack contains a covering cloaked img", () => {
    const wrapper = mount(el('<div><img></div>'))
    const img = wrapper.querySelector('img')!
    img.style.pointerEvents = 'none'
    img.getBoundingClientRect = rect(0, 0, 100, 100)
    expect(staleWhy(img, [mount(el('<div></div>'))], 50, 50)).toBe('not-in-stack')
  })
})
