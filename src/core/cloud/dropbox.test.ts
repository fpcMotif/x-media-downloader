...
const sessionStarted = (): Response => new Response(JSON.stringify({ session_id: 's' }), { status: 200 })
...
const dropboxRoute: Route = (endpoint, _init, body) => {
  if (endpoint === 'files/upload')
    return new Response(JSON.stringify({ id: 'id:simple', size: body?.byteLength ?? 0, path_display: '/alice/t1_0.mp4' }), { status: 200 })
  if (endpoint === 'files/upload_session/start') return new Response(JSON.stringify({ session_id: 'sess-1' }), { status: 200 })
  if (endpoint === 'files/upload_session/append_v2') return new Response('', { status: 200 })
  if (endpoint === 'files/upload_session/finish')
    return new Response(JSON.stringify({ id: 'id:session', size: 999, path_display: '/alice/big.mp4' }), { status: 200 })
  return new Response('unexpected', { status: 500 })
}

// Per-scenario routers (module scope: they capture no test state).
const routeMetaX: Route = () => new Response(JSON.stringify({ id: 'x', size: 1, path_display: '/p' }), { status: 200 })
...
const routeSessionStart500: Route = (endpoint) =>
  endpoint.endsWith('upload_session/start') ? new Response('no', { status: 500 }) : new Response('{}', { status: 200 })
...
const routeSingleChunkFinish500: Route = (endpoint) =>
  endpoint.endsWith('upload_session/start') ? sessionStarted() : new Response('commit failed', { status: 500 })
const routeSessionEmptyMeta: Route = (endpoint) =>
  endpoint.endsWith('upload_session/start') ? sessionStarted() : new Response(JSON.stringify({}), { status: 200 })
...
describe('DropboxUploader.upload', () => {
  it('maps a 410 source to sourceGone', async () => {
    const h = harness(dropboxRoute, sourceStub(() => sourceResponse(new Uint8Array(0), { status: 410 })))
    expect((await h.upload(input())).kind).toBe('sourceGone')
  })

  it('small media → single files/upload with an "add" commit', async () => {
    const h = harness(dropboxRoute, sourceStub(() => sourceResponse(new Uint8Array(2048))))
    const out = await h.upload(input())
    expect(out).toMatchObject({ kind: 'success', remoteId: 'id:simple', remotePath: '/alice/t1_0.mp4' })
  ...
  it('large/unknown media → session start → append(s) → finish in order', async () => {
    const h = harness(dropboxRoute, sourceStub(() => sourceResponse(new Uint8Array(20 * 1024 * 1024).fill(3), { contentLength: null })))
    ...
    expect(endpoints.filter((e) => e === 'files/upload_session/append_v2').length).toBeGreaterThanOrEqual(1)
    const finish = h.calls.at(-1)!
    expect((finish.arg as { cursor: { offset: number } }).cursor.offset).toBe(20 * 1024 * 1024 - finish.bodyLen)
  })

  it('escapes non-ASCII path chars in the Dropbox-API-Arg header', async () => {
    const h = harness(routeMetaX, sourceStub(() => sourceResponse(new Uint8Array(8))))
  ...
  it('empty source → failure', async () => {
    const h = harness(dropboxRoute, sourceStub(() => sourceResponse(new Uint8Array(0), { contentLength: 0 })))
  ...
  it('a small unknown-size source uses a single-chunk session (start close → finish empty)', async () => {
    const h = harness(dropboxRoute, sourceStub(() => sourceResponse(new Uint8Array(1024), { contentLength: null })))
    const out = await h.upload(input('alice/small.mp4'))
    expect(out).toMatchObject({ kind: 'success', remoteId: 'id:session' })
    expect(h.calls.map((c) => c.endpoint)).toEqual(['files/upload_session/start', 'files/upload_session/finish'])
  ...
  it('simple-upload HTTP error → failure (never throws)', async () => {
    const h = harness(routeSimple401, sourceStub(() => sourceResponse(new Uint8Array(64))))
    const out = await h.upload(input())
  ...
  it('session start failure → failure', async () => {
    const h = harness(routeSessionStart500, sourceStub(() => sourceResponse(new Uint8Array(20 * 1024 * 1024), { contentLength: null })))
    expect((await h.upload(input('alice/big.mp4'))).kind).toBe('failure')
  })

  it('session append failure → failure', async () => {
    const h = harness(routeSessionAppend503, sourceStub(() => sourceResponse(new Uint8Array(20 * 1024 * 1024), { contentLength: null })))
    expect((await h.upload(input('alice/big.mp4'))).kind).toBe('failure')
  })

  it('session finish failure → failure', async () => {
    const h = harness(routeSessionFinish500, sourceStub(() => sourceResponse(new Uint8Array(20 * 1024 * 1024), { contentLength: null })))
    expect((await h.upload(input('alice/big.mp4'))).kind).toBe('failure')
  })

  it('single-chunk session: a finish (commit) error → failure', async () => {
    const h = harness(routeSingleChunkFinish500, sourceStub(() => sourceResponse(new Uint8Array(1024), { contentLength: null })))
  ...
  it('session success falls back to byte count + request path when finish omits size/path/id', async () => {
    const h = harness(routeSessionEmptyMeta, sourceStub(() => sourceResponse(new Uint8Array(1024), { contentLength: null })))
    const out = await h.upload(input('alice/small.mp4'))
  ...
  it('falls back to local byte count + request path when metadata omits size/path', async () => {
    const h = harness(routeEmptyMeta, sourceStub(() => sourceResponse(new Uint8Array(2048))))
...
describe('DropboxUploader — empty-body edges', () => {
  it('declared non-zero length but empty simple body → failure', async () => {
    const h = harness(
      ...
      Layer.succeed(SourceFetch, {
        fetch: () =>
          Effect.succeed(
            new Response(emptyStream(), { status: 200, headers: { 'content-type': 'video/mp4', 'content-length': '7' } }),
          ),
      }),
    )
  ...
  it('empty stream with unknown size → session yields zero bytes → failure', async () => {
    const h = harness(
      dropboxRoute,
      Layer.succeed(SourceFetch, {
        fetch: () => Effect.succeed(new Response(emptyStream(), { status: 200, headers: { 'content-type': 'video/mp4' } })),
      }),
    )
    expect(await h.upload(input())).toMatchObject({ kind: 'failure', reason: 'empty source' })
  })
