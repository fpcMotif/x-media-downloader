...
const filesFound = (id: string): Response => new Response(JSON.stringify({ files: [{ id }] }), { status: 200 })
const folderResolved: Route = (url, init) =>
  url.includes('/drive/v3/files?') && isGet(init) ? filesFound('folder') : new Response('x', { status: 500 })

/** The standard Drive REST router (folders + multipart + resumable). */
const driveRoute: Route = (url, init) => {
  if (url.includes('uploadType=multipart')) return new Response(JSON.stringify({ id: 'file-mp' }), { status: 200 })
  if (url.includes('uploadType=resumable'))
    return new Response(null, { status: 200, headers: { location: 'https://session.example/put' } })
  if (url === 'https://session.example/put') return new Response(JSON.stringify({ id: 'file-rs' }), { status: 200 })
  if (url.includes('/drive/v3/files?') && isGet(init)) return new Response(JSON.stringify({ files: [] }), { status: 200 })
  if (url.includes('/drive/v3/files?')) return new Response(JSON.stringify({ id: 'folder-1' }), { status: 200 })
...
const routeFolderCreateFails: Route = (url, init) =>
  isGet(init) ? new Response(JSON.stringify({ files: [] }), { status: 200 }) : new Response('boom', { status: 500 })
const routeMultipart403: Route = (url, init) =>
  url.includes('uploadType=multipart') ? new Response('denied', { status: 403 }) : folderResolved(url, init)
const routeNoSessionUrl: Route = (url, init) =>
  url.includes('uploadType=resumable') ? new Response(null, { status: 200 }) : folderResolved(url, init)
const routeInitiate500: Route = (url, init) =>
  url.includes('uploadType=resumable') ? new Response('no', { status: 500 }) : folderResolved(url, init)
const routeFinalPutNoId: Route = (url, init) => {
  if (url.includes('uploadType=resumable')) return new Response(null, { status: 200, headers: { location: 'https://s/put' } })
...
const routeMidChunk500: Route = (url, init) => {
  ...
  if (url === 'https://s/put') {
    ...
    return range.endsWith('/*') ? new Response('mid-fail', { status: 500 }) : new Response(JSON.stringify({ id: 'x' }), { status: 200 })
  }
  return folderResolved(url, init)
}
const routeErrTextRejects: Route = (url, init) =>
  isGet(init)
    ...
    : // POST (folder create) errors AND its body read rejects (dropped connection)
      ({ ok: false, status: 500, text: () => Promise.reject(new Error('read failed')), headers: new Headers() } as unknown as Response)
...
const harness = (route: Route, src: Layer.Layer<SourceFetch>) => {
  ...
  const fetchLayer = Layer.succeed(FetchService, {
    ...
    fetchPromise: (async (url: string | URL, init?: RequestInit) => record(String(url), init)) as typeof fetch,
  })
  const app = DriveUploaderLive.pipe(Layer.provide(Layer.mergeAll(fetchLayer, src, FolderCacheLive)))
  ...
  return {
    ...
    ensureRoot: (token = 'AT') => rt.runPromise(Effect.flatMap(DriveUploader, (u) => u.ensureRoot(token))),
  }
}

const folderCreates = (calls: Call[]): Call[] =>
  calls.filter((c) => c.url.includes('/drive/v3/files?') && !c.url.includes('/upload/') && c.method === 'POST')

describe('DriveUploader.upload', () => {
  it('maps a 403 source to sourceGone (link-rot), not a failure', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(0), { status: 403 })))
    expect((await h.upload(input())).kind).toBe('sourceGone')
  })

  it('maps a 500 source to a (retryable) failure', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(0), { status: 500 })))
    expect((await h.upload(input())).kind).toBe('failure')
  })

  it('small media → one multipart request, returns the file id', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(1024).fill(7))))
    const out = await h.upload(input())
    expect(out).toMatchObject({ kind: 'success', bytes: 1024, remoteId: 'file-mp', remotePath: 'alice/t1_0.jpg' })
  ...
  it('unknown/large media → resumable session, returns the file id', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(300).fill(9), { contentLength: null })))
    const out = await h.upload(input())
  ...
  it('empty source → failure (never a fake save)', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(0), { contentLength: 0 })))
    expect((await h.upload(input())).kind).toBe('failure')
  })

  it('caches the per-handle subfolder across uploads (one runtime = one Ref)', async () => {
    const h = harness(driveRoute, sourceStub(() => sourceResponse(new Uint8Array(16))))
...
describe('DriveUploader.ensureRoot', () => {
  it('resolves a top-level folder (no parent constraint in the query)', async () => {
    const h = harness(routeRootFound, sourceStub(() => sourceResponse(new Uint8Array(1))))
  ...
  it('escapes single-quotes and backslashes in the folder name (q= injection guard)', async () => {
    const h = harness(routeFolderX, sourceStub(() => sourceResponse(new Uint8Array(16))))
    await h.upload({
      url: 'https://pbs.twimg.com/media/x.jpg',
      target: { path: "a'b\\c/f.jpg", handle: "a'b\\c", filename: 'f.jpg', contentType: 'image/jpeg' },
    })
    const q = decodeURIComponent(new URL(h.calls[0]!.url).searchParams.get('q') ?? '')
  ...
  it('reuses an existing folder instead of creating a duplicate', async () => {
    const h = harness(routeFolderExisting, sourceStub(() => sourceResponse(new Uint8Array(16))))
    await h.upload(input())
...
describe('DriveUploader — error & resumable edge paths', () => {
  it('maps a folder-resolution failure to an upload failure (never throws)', async () => {
    const h = harness(routeFolderCreateFails, sourceStub(() => sourceResponse(new Uint8Array(64))))
    const out = await h.upload(input())
  ...
  it('multipart upload failure → failure outcome', async () => {
    const h = harness(routeMultipart403, sourceStub(() => sourceResponse(new Uint8Array(64))))
    expect((await h.upload(input())).kind).toBe('failure')
  })

  it('resumable initiate without a session url → failure', async () => {
    const h = harness(routeNoSessionUrl, sourceStub(() => sourceResponse(new Uint8Array(64), { contentLength: null })))
    const out = await h.upload(input())
    expect(out).toMatchObject({ kind: 'failure' })
  ...
  it('resumable initiate failure → failure', async () => {
    const h = harness(routeInitiate500, sourceStub(() => sourceResponse(new Uint8Array(64), { contentLength: null })))
    expect((await h.upload(input())).kind).toBe('failure')
  })

  it('resumable: final PUT returning no id → failure', async () => {
    const h = harness(routeFinalPutNoId, sourceStub(() => sourceResponse(new Uint8Array(64), { contentLength: null })))
    const out = await h.upload(input())
    expect(out).toMatchObject({ kind: 'failure' })
  ...
  it('resumable: final PUT error status → failure', async () => {
    const h = harness(routeFinalPut410, sourceStub(() => sourceResponse(new Uint8Array(64), { contentLength: null })))
    expect((await h.upload(input())).kind).toBe('failure')
  })
  ...
  it('resumable multi-chunk: 308 on non-final chunk then 200 on the last', async () => {
    ...
    const route: Route = (url, init) => {
      ...
      if (url === 'https://s/put') {
        const range = ((init?.headers ?? {}) as Record<string, string>)['content-range'] ?? ''
        puts.push(range)
        return range.endsWith('/*') ? new Response(null, { status: 308 }) : new Response(JSON.stringify({ id: 'big-file' }), { status: 200 })
      }
      return folderResolved(url, init)
    }
    // 9 MiB, known size > SIMPLE_MAX_BYTES → resumable in 8 MiB chunks (8 MiB + 1 MiB).
    const h = harness(route, sourceStub(() => sourceResponse(new Uint8Array(9 * 1024 * 1024))))
  ...
  it('resumable: a 500 on a non-final chunk → failure', async () => {
    const h = harness(routeMidChunk500, sourceStub(() => sourceResponse(new Uint8Array(9 * 1024 * 1024))))
  ...
  it('empty stream with unknown size → resumable yields zero bytes → failure', async () => {
    const h = harness(
      ...
      Layer.succeed(SourceFetch, {
        fetch: () => Effect.succeed(new Response(emptyStream(), { status: 200, headers: { 'content-type': 'image/jpeg' } })),
      }),
    )
    const out = await h.upload(input())
  ...
  it('declared non-zero length but empty multipart body → failure', async () => {
    const h = harness(
      ...
      Layer.succeed(SourceFetch, {
        fetch: () =>
          Effect.succeed(
            new Response(emptyStream(), { status: 200, headers: { 'content-type': 'image/jpeg', 'content-length': '5' } }),
          ),
      }),
    )
  ...
  it("errText falls back to '' when the error body read itself fails", async () => {
    const h = harness(routeErrTextRejects, sourceStub(() => sourceResponse(new Uint8Array(32))))
    const out = await h.upload(input())
    expect(out).toMatchObject({ kind: 'failure' })
