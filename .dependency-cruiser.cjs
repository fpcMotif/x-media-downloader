...
module.exports = {
  forbidden: [
    ...
    },
    {
      name: "no-circular",
      comment:
        "No dependency cycles, anywhere in src — type-only imports included (tsPreCompilationDeps).",
      severity: "error",
      from: {},
      to: { circular: true },
    },
  ...
  options: {
    ...
    // tsconfig.depcruise.json re-anchors WXT's '@'/'~' → src path aliases at the
    // repo root: the real tsconfig inherits them from .wxt/tsconfig.json behind an
    // "extends" whose relative baseUrl the resolver mis-anchors, silently dropping
    // every aliased edge from the graph — and with it, the rules' teeth.
    tsConfig: { fileName: "tsconfig.depcruise.json" },
    // Cruise source-level imports: without this, TypeScript elides unused and
    // type-only imports before dependency-cruiser sees them, so deep imports
    // of those shapes would slip through the boundary rules.
